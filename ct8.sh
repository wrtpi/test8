...
if [ "$hona" = "ct8" ]; then
address="ct8.pl"
keep_path="${HOME}/domains/${snb}.${USERNAME}.ct8.pl/public_nodejs"
...
read_reym() {
yellow "方式一：(推荐)使用ct8/Hostuno自带域名，不支持proxyip功能：输入回车"
...
purple "检测到ct8/Hostuno-sb-yg脚本已安装，执行端口替换，请稍等……"
...
if [ "$hona" = "ct8" ]; then
sed -i '' -e "17s|'$vlp'|'$vless_port'|" ct8keep.sh
sed -i '' -e "18s|'$vmp'|'$vmess_port'|" ct8keep.sh
sed -i '' -e "19s|'$hyp'|'$hy2_port'|" ct8keep.sh
fi
resservsb
green "端口替换完成！"
...
sleep 2
	...
	cd
        purple "************************************************************"
        purple "ct8/Hostuno-sb-yg脚本安装结束"
	purple "退出SSH"
  ...
  reading "\n确定要卸载吗？【y/n】: " choice
    case "$choice" in
	  ...
	  bash -c 'ps aux | grep $(whoami) | grep -v "sshd\|bash\|grep" | awk "{print \$2}" | xargs -r kill -9 >/dev/null 2>&1' >/dev/null 2>&1
          rm -rf bin domains ct8keep.sh webport.sh
	  devil www list | awk 'NR > 1 && NF {print $1}' | xargs -I {} devil www del {} > /dev/null 2>&1
	  sed -i '' '/export PATH="\$HOME\/bin:\$PATH"/d' ~/.bashrc
          source ~/.bashrc
          purple "************************************************************"
          purple "ct8/Hostuno-sb-yg卸载完成！"
          purple "欢迎继续使用脚本：bash <(curl -Ls https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/ct8.sh)"
          purple "************************************************************"
          ;;
        [Nn]) exit 0 ;;
...
reading "\n注意！！！清理所有进程并清空所有安装内容，将退出ssh连接，确定继续清理吗？【y/n】: " choice
  case "$choice" in
    ...
    source ~/.bashrc
    purple "************************************************************"
    purple "ct8/Hostuno-sb-yg清理重置完成！"
    purple "欢迎继续使用脚本：bash <(curl -Ls https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/ct8.sh)"
    purple "************************************************************"
...
FILE_INFO=("https://github.com/yonggekkk/Cloudflare_vless_trojan/releases/download/ct8/sb web" "https://github.com/yonggekkk/Cloudflare_vless_trojan/releases/download/ct8/server bot")
declare -A FILE_MAP
...
yellow "2、选择9重置"
yellow "3、当前ct8/Hostuno服务器炸了？等会再试"
...
注：如果ct8/Hostuno的IP被墙，proxyip依旧有效，但用于客户端地址的非标端口反代IP将不可用
注：可能有大佬会扫ct8/Hostuno的反代IP作为其共享IP库或者出售，请慎重将reality域名设置为CF域名
-------------------------------------------------------------------------------------------------
...
servkeep() {
sed -i '' -e "14s|''|'$UUID'|" ct8keep.sh
sed -i '' -e "17s|''|'$vless_port'|" ct8keep.sh
sed -i '' -e "18s|''|'$vmess_port'|" ct8keep.sh
sed -i '' -e "19s|''|'$hy2_port'|" ct8keep.sh
sed -i '' -e "20s|''|'$IP'|" ct8keep.sh
sed -i '' -e "21s|''|'$reym'|" ct8keep.sh
if [ ! -f "$WORKDIR/boot.log" ]; then
sed -i '' -e "15s|''|'${ARGO_DOMAIN}'|" ct8keep.sh
sed -i '' -e "16s|''|'${ARGO_AUTH}'|" ct8keep.sh
fi
echo '#!/bin/bash
red() { echo -e "\e[1;91m$1\033[0m"; }
green() { echo -e "\e[1;32m$1\033[0m"; }
...
purple() { echo -e "\e[1;35m$1\033[0m"; }
USERNAME=$(whoami | tr '\''[:upper:]'\'' '\''[:lower:]'\'')
WORKDIR="${HOME}/domains/${USERNAME}.ct8.pl/logs"
snb=$(hostname | cut -d. -f1)
hona=$(hostname | cut -d. -f2)
' > webport.sh
...
if [[ -e $WORKDIR/config.json ]]; then
  ...
  mkdir -p "$HOME/bin"
  curl -Ls https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/ct8.sh > "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
...
if [ "$hona" = "ct8" ]; then
curl -sL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/app.js -o "$keep_path"/app.js
...
curl -sSL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/ct8keep.sh -o ct8keep.sh && chmod +x ct8keep.sh
fi
curl -sL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/index.html -o "$FILE_PATH"/index.html
curl -sL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/sversion | awk -F "更新内容" '{print $1}' | head -n 1 > $WORKDIR/v
else
red "未安装脚本，请选择1进行安装" && exit
fi
}
...
if [ "$hona" = "ct8" ]; then
sed -i '' -e "15s|''|'$(cat ARGO_DOMAIN_show.log 2>/dev/null)'|" ~/ct8keep.sh
sed -i '' -e "16s|''|'$(cat ARGO_AUTH_show.log 2>/dev/null)'|" ~/ct8keep.sh
fi
...
menu() {
   ...
   green "ct8/Hostuno三协议共存脚本：vless-reality/Vmess-ws(Argo)/Hy2"
   green "脚本快捷方式：sb"
   echo   "============================================================"
   green  "1. 一键安装 ct8/Hostuno-sb-yg"
   echo   "------------------------------------------------------------"
   yellow "2. 卸载删除 ct8/Hostuno-sb-yg"
   echo   "------------------------------------------------------------"
   green  "3. 重启主进程 (修复主节点)"
   echo   "------------------------------------------------------------"
...
if [ "$hona" = "ct8" ]; then
red "目前免费ct8使用代理脚本会有被封账号的风险，请知晓！！！"
fi
...
latestV=$(curl -sL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/sversion | awk -F "更新内容" '{print $1}' | head -n 1)
if [ -f $WORKDIR/v ]; then
if [ "$insV" = "$latestV" ]; then
echo -e "当前 ct8/Hostuno-sb-yg 脚本最新版：${purple}${insV}${re} (已安装)"
else
echo -e "当前 ct8/Hostuno-sb-yg 脚本版本号：${purple}${insV}${re}"
echo -e "检测到最新 ct8/Hostuno-sb-yg 脚本版本号：${yellow}${latestV}${re} (可选择5进行更新)"
echo -e "${yellow}$(curl -sL https://raw.githubusercontent.com/wrtpi/test8/refs/heads/main/sversion)${re}"
fi
echo -e "========================================================="
...
echo -e "当前 ct8/Hostuno-sb-yg 脚本版本号：${purple}${latestV}${re}"
yellow "未安装 ct8/Hostuno-sb-yg 脚本！请选择 1 安装"
fi
   echo -e "========================================================="
